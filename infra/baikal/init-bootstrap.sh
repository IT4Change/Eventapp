...
# non-zero exit here would loop the container. The actual DAV user is
# provisioned host-side via `npm run cli:baikal:bootstrap`.
# (Pattern adapted from webcraftmedia/jahrweiser.)
...
# php-fpm in this image runs as the 'nginx' user; chown so Baikal can write
# the SQLite file.
...
echo "[baikal-init] next: from host, run 'npm run cli:baikal:bootstrap'"
exit 0
